...
class PureEdDSA(object):
    ...
    def sign(self, privkey, pubkey, msg, ctx, hflag):
        ...
        # Calculate h.
        khash = self.H(privkey, None, None)
        s = from_le(self.__clamp(khash[:self.b // 8]))
        h = from_le(self.H(R + pubkey + msg, ctx, hflag)) % self.l
        # Calculate s.
        S = to_bytes(((r + h * s) % self.l), self.b // 8, byteorder="little")
...
def test():
    ...
    print("prv2 = %s" % b64encode(prv), file=sys.stderr)
    print("pub2 = %s" % b64encode(pub))
...
def attack():
    ...
    empty_sign = b64decode(b'T0vUg8CHzYIJupRYQEMWQeLy6bgEkJYJngFUpwbTg1wFAGppiiUmn40t32ALaQqVUjFpsGgBtQWyJQnSeVTOBA==')
    ...
    # SIGNATURE Q2 : b'T0vUg8CHzYIJupRYQEMWQeLy6bgEkJYJngFUpwbTg1wbmBv8SQtZzmUPIvukcfDivEAP5Ga9R4AyswWWZ//GAA=='
