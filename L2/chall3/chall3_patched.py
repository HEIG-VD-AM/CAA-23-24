...
class PureEdDSA(object):
    ...
    def sign_date(self, privkey, pubkey, msg, ctx, hflag):
        ...
        r = from_le(self.H(seed + msg + date, ctx, hflag)) % self.l
        R = (self.B * r).encode()
...
def test():
    ...
    # Forging signature
    R = (B * r).encode()
    h = from_le(Ed25519_inthash(R + pub + forged_msg + date, None, None)) % l
    S = to_bytes(((r + h * s) % l), b // 8, byteorder="little")
    forged_sig = R + S
    print(ed.verify_date(pub, forged_msg, date, forged_sig))
    ...
    # (sig, date) = (
    # b64decode(b'MExjz6t6pAdGMt+Chj+1ss0xQirh/yTd0/YmrjoXzYzdcga0QM8kDECDv9zSUDLJ8JKgDAlbCmD1VmT8XsoMDw=='),
    # b'2023-11-10 19:47:00.135606')
    # (sig2, date2) = (
    # b64decode(b'MExjz6t6pAdGMt+Chj+1ss0xQirh/yTd0/YmrjoXzYz0x4puYbEl3che3Z6L2mrU+pGk8R5movsZ4Z8XIGMlCA=='),
    # b'2023-11-10 19:47:30.430173')
    # msg = b"Hello World!"
    # forged_msg = b"Grade of Alexandre Duc at CAA = 6.0"
    # l = hexi("1000000000000000000000000000000014def9dea2f79cd65812631a5cf5d3ed")
    # b = 256
